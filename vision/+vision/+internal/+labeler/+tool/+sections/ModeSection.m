...
% This class defines the videoLabeler mode section which houses the
% buttons for zooming, panning and ROI mode selection.
...
classdef ModeSection < vision.internal.uitools.NewToolStripSection
    ...
    methods
        ...
        function setROIIcon(this,mode)
            % Set Icon for ROI button
            % if mode is 'pixel', then set the pixel label icon, otherwise
            % use the rectangle icon as default
            if strcmp(mode,'pixel')
                iconPath = fullfile(matlabroot,'toolbox','vision','vision','+vision','+internal','+labeler','+tool','+icons','pixel_label_24.png');
            else
                iconPath = fullfile(matlabroot,'toolbox','vision','vision','+vision','+internal','+labeler','+tool','+icons','ROI_Rectangle_sm_24px.png');
            end
