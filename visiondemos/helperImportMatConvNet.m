...
layersMap = createLayerMap();
externalLayers = layersMap.externalLayers( layers );
net = SeriesNetwork(externalLayers);
...
layer = nnet.internal.cnn.layer.Convolution2D(name, filterSize, ...
    numChannels, numFilters, stride, 'manual', padding);
...
map = nnet.internal.cnn.layer.util.InternalExternalMap( layers );
