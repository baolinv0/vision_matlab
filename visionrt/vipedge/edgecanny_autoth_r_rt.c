/*
 *  EDGECANNY_AUTOTH_R_RT Helper function for Edge block (Canny method).
 *
 *  Copyright 1995-2005 The MathWorks, Inc.
 */
#include "vipedge_rt.h"  

static void TrackNonRecursive_AR ( boolean_T *edge, 
                                real32_T  *tmpOrMag,
                                int16_T   *stackR,
                                int16_T   *stackC,
                                int16_T   *stackP,
...
static void EstimateAutoThreshold_R (real32_T *tmpOrMag, 
                               real32_T *highTh, 
                               real32_T *lowTh,
                               int_T inpRows,
                               int_T inpCols,
                         ...
                         const real32_T *autoPercent)
{
    ...
    real32_T PercentOfPixelsNotEdges = autoPercent[0]/100.0F;  
    real32_T ThresholdRatio          = 0.4F; /* Used for selecting thresholds */
    int_T inpWidth                   = inpRows*inpCols;
    real32_T thresh                  = PercentOfPixelsNotEdges*inpWidth;
    ...
    for (r=quarterFiltLen; r<inpRows-quarterFiltLen; r++)
      ...
      {
        real32_T valBasedIdx = (tmpOrMag[r+c*inpRows] * N_BINS_MIN1);   
...
static void HysteresisThresholding_AR (boolean_T *edge, 
                                    real32_T  *tmpOrMag,
                                    int16_T   *stackR,
                                    int16_T   *stackC,
                                    int16_T   *stackP,
                              ...
                              const real32_T  *autoPercent)
{
    ...
    EstimateAutoThreshold_R (tmpOrMag, &highTh, &lowTh,inpRows,
                           inpCols,quarterFiltLen,autoPercent);

    for (r=0; r<inpRows; r++)
      for (c=0; c<inpCols; c++)
        if (tmpOrMag[r+c*inpRows] >= highTh)
             TrackNonRecursive_AR (edge,tmpOrMag,stackR,stackC,stackP,stackQ,
          ...
          int_T  halfFiltLen)
{
    ...
    MWVIP_RC_Gaussian_Smoothing_R(inpImg,gauss1D,cFiltered,rFiltered,inpRows,inpCols,halfFiltLen);

    /* step-2: filter smoothed image along column with the derivative of Gaussian filter */
    MWVIP_C_Derivative_Image_R(cFiltered,dgauss1D,tmpOrMag,inpRows,inpCols,halfFiltLen);
    memcpy((byte_T *)cFiltered,(byte_T *)tmpOrMag,inpWidth*sizeof(real32_T));

    /* step-3: filter smoothed image along row with the derivative of Gaussian filter */
    MWVIP_R_Derivative_Image_R(rFiltered,dgauss1D,tmpOrMag,inpRows,inpCols,halfFiltLen);
    memcpy((byte_T *)rFiltered,(byte_T *)tmpOrMag,inpWidth*sizeof(real32_T));

    /* step-4: get the magnitude of cFilt and rFilt with non-maximum suppression */
    MWVIP_NonMaximum_Suppression_R(cFiltered,rFiltered,tmpOrMag,inpRows,inpCols); 
    ...
    HysteresisThresholding_AR (outEdge,tmpOrMag,stackR,stackC,stackP,stackQ,
