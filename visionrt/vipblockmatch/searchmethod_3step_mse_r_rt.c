						                                                                     /*
 *  SEARCHMETHOD_3STEP_MSE_R_RT Helper function for Block Matching block.
 *
 *  Copyright 1995-2005 The MathWorks, Inc.
 */
#include "vipblockmatch_rt.h"  

LIBMWVISIONRT_API void MWVIP_SearchMethod_3Step_MSE_R(const real32_T *blkCS, const real32_T *blkPB, /* CS = current image (smaller block), PB = previous image (bigger block) */
                                             int_T rowsImgCS,   int_T rowsImgPB,  
                                             int_T blkCSWidthX, int_T blkCSHeightY,  
                                             int_T blkPBWidthX, int_T blkPBHeightY,  
                                             int_T *xIdx,         int_T *yIdx)
{
    const int_T endRowIdx = blkPBHeightY-blkCSHeightY+1;
    const int_T endColIdx = blkPBWidthX -blkCSWidthX +1;
    real32_T sum3= 0.0;  /* holds the minimum value */
    int_T p;
    int_T flag = 0;
    int_T midIdxR = endRowIdx/2;
    int_T midIdxC = endColIdx/2;
    int_T range = MIN(midIdxR,midIdxC);

    int_T delta = (int_T)(range/2) + 1;
    while (delta > 0) {
      int_T iy = MAX((midIdxC - delta),0);
      int_T ix = MAX((midIdxR - delta),0);
      int_T colPts = 3;
      p = iy;
      while (colPts--) {
        int_T rowPts = 3;
        int_T q = ix;
        while (rowPts--) {
          real32_T sum2 = 0.0;
          int_T idxCol_PB = rowsImgPB*p; 
          int_T idxCS = 0;
          int_T m;
          for (m = 0; m < blkCSWidthX; m++) {
            int_T n;
            for (n = 0; n < blkCSHeightY; n++) {
              int_T idxPB = q + n + idxCol_PB;  
              const real32_T valPB = blkPB[idxPB];  /* PB = previous image (bigger block) */
              const real32_T valCS = blkCS[idxCS+n];/* CS = current image (smaller block) */
              const real32_T dist = valPB-valCS;
              /* accumulate sum of absolute differences */
              sum2 +=   dist*dist;
            }
            idxCS += rowsImgCS;
            idxCol_PB += rowsImgPB;  
          }
          /* Store the new minimum and get the corresponding indices. */
          if (flag == 0) {
            sum3 = sum2;
            yIdx[0] = q;
            xIdx[0] = p;
            flag = 1;
          } else {
            if (sum2 < sum3) {
              sum3 = sum2;
              yIdx[0] = q;
              xIdx[0] = p;
            }
          }
          q += delta;
          while ((rowPts > 0) && (q >= endRowIdx)) q--;
        }
        p += delta;
        while ((colPts > 0) && (p >= endColIdx)) p--;
      }
      midIdxC = xIdx[0];
      midIdxR = yIdx[0];
      delta--;
    }
}

/* [EOF] searchmethod_3step_mse_r_rt.c */
