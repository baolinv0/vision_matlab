                                                                     /*
 *  SEARCHMETHOD_3STEP_MAD_R_RT Helper function for Block Matching block.
 *
 *  Copyright 1995-2005 The MathWorks, Inc.
 */
#include "vipblockmatch_rt.h"  

LIBMWVISIONRT_API void MWVIP_SearchMethod_3Step_MAD_R(const real32_T *blkCS, const real32_T *blkPB, /* CS = current image (smaller block), PB = previous image (bigger block) */
                                             ...
                                             int_T *xIdx,         int_T *yIdx)
{
    ...
    real32_T sum3= 0.0;  /* holds the minimum value */
    int_T p;
    int_T flag = 0;
        ...
        while (rowPts--) {
          ...
          for (m = 0; m < blkCSWidthX; m++) {
            ...
            for (n = 0; n < blkCSHeightY; n++) {
              ...
              const real32_T valPB = blkPB[idxPB];  /* PB = previous image (bigger block) */
              const real32_T valCS = blkCS[idxCS+n];/* CS = current image (smaller block) */
              const real32_T dist = valPB-valCS;
...
/* [EOF] searchmethod_3step_mad_r_rt.c */
