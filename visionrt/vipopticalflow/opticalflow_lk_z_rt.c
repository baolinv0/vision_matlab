/*
*  OPTICALFLOW_LK_Z_RT runtime function for Optical Flow Block
                                       ...
                                       int_T  inCols)
{
    ...
    real_T gauss1DFilt[5] = {0.0625F,0.25F,0.375F,0.25F,0.0625F};
    ...
    for( colIdx = 0; colIdx < inCols; colIdx++ )
    {
        real_T *tmpWGradRR = (real_T *)outVel;
        real_T *tmpWGradCC = tmpWGradRR + inRows;
        real_T *tmpWGradRC = tmpWGradCC + inRows;
        real_T *tmpWGradRT = tmpWGradRC + inRows;
        real_T *tmpWGradCT = tmpWGradRT + inRows;
    ...
    for( colIdx = 0; colIdx < inCols; colIdx++ )
    {
        ...
        for( j = 0; j < inRows; j++ )
        {
            ...
            /************************************************************************/
            {
                ...
                real_T B = 4.0*WWGradRC*WWGradRC + tmp*tmp;
                ...
                if ((eig1 >= threshEigen) && (eig2 >= threshEigen) && (fabs(delta)>=THRESH_ABS_DELTA))
                {
                    ...
                    real_T deltaC = -(WWGradRT * WWGradRC - WWGradCT * WWGradRR);
                    real_T deltaR = -(WWGradRC * WWGradCT - WWGradCC * WWGradRT);
