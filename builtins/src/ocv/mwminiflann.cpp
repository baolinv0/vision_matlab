...
static cvmwflann::IndexParams& get_params(const cv::mwflann::IndexParams& p)
{
    return *(cvmwflann::IndexParams*)(p.params);
}

cv::mwflann::IndexParams::~IndexParams()
{
...
using namespace cvmwflann;

IndexParams::IndexParams()
{
    params = new ::cvmwflann::IndexParams();
}

template<typename T>
T getParam(const IndexParams& _p, const std::string& key, const T& defaultVal=T())
{
    ::cvmwflann::IndexParams& p = get_params(_p);
    ::cvmwflann::IndexParams::const_iterator it = p.find(key);
    if( it == p.end() )
        return defaultVal;
...
{
    setParam(*this, "algorithm", (cvmwflann::flann_algorithm_t)value);
}
            ...
            std::vector<double>& numValues) const
{
    ...
    ::cvmwflann::IndexParams& p = get_params(*this);
    ::cvmwflann::IndexParams::const_iterator it = p.begin(), it_end = p.end();
    ...
    for( ; it != it_end; ++it )
    {
        ...
        try
        {
            cvmwflann::flann_algorithm_t val = it->second.cast<cvmwflann::flann_algorithm_t>();
...
buildIndex_(void*& index, const Mat& data, const IndexParams& params, const Distance& dist = Distance())
{
    ...
    ::cvmwflann::Matrix<ElementType> dataset((ElementType*)data.data, data.rows, data.cols);
...
{
    buildIndex_<Distance, ::cvmwflann::Index<Distance> >(index, data, params, dist);
}

#if CV_NEON
typedef ::cvmwflann::Hamming<uchar> HammingDistance;
#else
typedef ::cvmwflann::HammingLUT2 HammingDistance;
#endif
...
void Index::build(InputArray _data, const IndexParams& params, flann_distance_t _distType)
{
    ...
    case FLANN_DIST_HAMMING:
        ...
        break;
    case FLANN_DIST_L2:
        buildIndex< ::cvmwflann::L2<float> >(index, data, params);
        break;
    case FLANN_DIST_L1:
        buildIndex< ::cvmwflann::L1<float> >(index, data, params);
        break;
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
        buildIndex< ::cvmwflann::MaxDistance<float> >(index, data, params);
        break;
    case FLANN_DIST_HIST_INTERSECT:
        buildIndex< ::cvmwflann::HistIntersectionDistance<float> >(index, data, params);
        break;
    case FLANN_DIST_HELLINGER:
        buildIndex< ::cvmwflann::HellingerDistance<float> >(index, data, params);
        break;
    case FLANN_DIST_CHI_SQUARE:
        buildIndex< ::cvmwflann::ChiSquareDistance<float> >(index, data, params);
        break;
    case FLANN_DIST_KL:
        buildIndex< ::cvmwflann::KL_Divergence<float> >(index, data, params);
        break;
#endif
    default:
...
{
    deleteIndex_< ::cvmwflann::Index<Distance> >(index);
}
...
void Index::release()
{
    ...
    switch( distType )
    {
        case FLANN_DIST_HAMMING:
            ...
            break;
        case FLANN_DIST_L2:
            deleteIndex< ::cvmwflann::L2<float> >(index);
            break;
        case FLANN_DIST_L1:
            deleteIndex< ::cvmwflann::L1<float> >(index);
            break;
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
        case FLANN_DIST_MAX:
            deleteIndex< ::cvmwflann::MaxDistance<float> >(index);
            break;
        case FLANN_DIST_HIST_INTERSECT:
            deleteIndex< ::cvmwflann::HistIntersectionDistance<float> >(index);
            break;
        case FLANN_DIST_HELLINGER:
            deleteIndex< ::cvmwflann::HellingerDistance<float> >(index);
            break;
        case FLANN_DIST_CHI_SQUARE:
            deleteIndex< ::cvmwflann::ChiSquareDistance<float> >(index);
            break;
        case FLANN_DIST_KL:
            deleteIndex< ::cvmwflann::KL_Divergence<float> >(index);
            break;
#endif
        default:
            CV_Error(CV_StsBadArg, "Unknown/unsupported distance type");
                  ...
                  int knn, const SearchParams& params)
{
    ...
    ::cvmwflann::Matrix<ElementType> _query((ElementType*)query.data, query.rows, query.cols);
    ::cvmwflann::Matrix<int> _indices((int*)indices.data, indices.rows, indices.cols);
    ::cvmwflann::Matrix<DistanceType> _dists((DistanceType*)dists.data, dists.rows, dists.cols);

    ((IndexType*)index)->knnSearch(_query, _indices, _dists, knn,
                                   (const ::cvmwflann::SearchParams&)get_params(params));
}
...
void runKnnSearch(void* index, const Mat& query, Mat& indices, Mat& dists,
                  int knn, const SearchParams& params)
{
    runKnnSearch_<Distance, ::cvmwflann::Index<Distance> >(index, query, indices, dists, knn, params);
}
...
int runRadiusSearch(void* index, const Mat& query, Mat& indices, Mat& dists,
                     double radius, const SearchParams& params)
{
    return runRadiusSearch_<Distance, ::cvmwflann::Index<Distance> >(index, query, indices, dists, radius, params);
}
               ...
               OutputArray _dists, int knn, const SearchParams& params)
{
    ...
    case FLANN_DIST_HAMMING:
        ...
        break;
    case FLANN_DIST_L2:
        runKnnSearch< ::cvmwflann::L2<float> >(index, query, indices, dists, knn, params);
        break;
    case FLANN_DIST_L1:
        runKnnSearch< ::cvmwflann::L1<float> >(index, query, indices, dists, knn, params);
        break;
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
        runKnnSearch< ::cvmwflann::MaxDistance<float> >(index, query, indices, dists, knn, params);
        break;
    case FLANN_DIST_HIST_INTERSECT:
        runKnnSearch< ::cvmwflann::HistIntersectionDistance<float> >(index, query, indices, dists, knn, params);
        break;
    case FLANN_DIST_HELLINGER:
        runKnnSearch< ::cvmwflann::HellingerDistance<float> >(index, query, indices, dists, knn, params);
        break;
    case FLANN_DIST_CHI_SQUARE:
        runKnnSearch< ::cvmwflann::ChiSquareDistance<float> >(index, query, indices, dists, knn, params);
        break;
    case FLANN_DIST_KL:
        runKnnSearch< ::cvmwflann::KL_Divergence<float> >(index, query, indices, dists, knn, params);
        break;
#endif
    default:
        CV_Error(CV_StsBadArg, "Unknown/unsupported distance type");
                        ...
                        const SearchParams& params)
{
    ...
    case FLANN_DIST_L2:
        return runRadiusSearch< ::cvmwflann::L2<float> >(index, query, indices, dists, radius, params);
    case FLANN_DIST_L1:
        return runRadiusSearch< ::cvmwflann::L1<float> >(index, query, indices, dists, radius, params);
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
        return runRadiusSearch< ::cvmwflann::MaxDistance<float> >(index, query, indices, dists, radius, params);
    case FLANN_DIST_HIST_INTERSECT:
        return runRadiusSearch< ::cvmwflann::HistIntersectionDistance<float> >(index, query, indices, dists, radius, params);
    case FLANN_DIST_HELLINGER:
        return runRadiusSearch< ::cvmwflann::HellingerDistance<float> >(index, query, indices, dists, radius, params);
    case FLANN_DIST_CHI_SQUARE:
        return runRadiusSearch< ::cvmwflann::ChiSquareDistance<float> >(index, query, indices, dists, radius, params);
    case FLANN_DIST_KL:
        return runRadiusSearch< ::cvmwflann::KL_Divergence<float> >(index, query, indices, dists, radius, params);
#endif
    default:
        CV_Error(CV_StsBadArg, "Unknown/unsupported distance type");
    }
...
template<typename IndexType> void saveIndex_(const Index* index0, const void* index, FILE* fout)
{
    ...
    ::cvmwflann::save_header(fout, *_index);
...
void Index::save(const std::string& filename) const
{
    ...
    case FLANN_DIST_HAMMING:
        ...
        break;
    case FLANN_DIST_L2:
        saveIndex< ::cvmwflann::L2<float> >(this, index, fout);
        break;
    case FLANN_DIST_L1:
        saveIndex< ::cvmwflann::L1<float> >(this, index, fout);
        break;
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
        saveIndex< ::cvmwflann::MaxDistance<float> >(this, index, fout);
        break;
    case FLANN_DIST_HIST_INTERSECT:
        saveIndex< ::cvmwflann::HistIntersectionDistance<float> >(this, index, fout);
        break;
    case FLANN_DIST_HELLINGER:
        saveIndex< ::cvmwflann::HellingerDistance<float> >(this, index, fout);
        break;
    case FLANN_DIST_CHI_SQUARE:
        saveIndex< ::cvmwflann::ChiSquareDistance<float> >(this, index, fout);
        break;
    case FLANN_DIST_KL:
        saveIndex< ::cvmwflann::KL_Divergence<float> >(this, index, fout);
        break;
#endif
    default:
        fclose(fout);
...
{
    return loadIndex_<Distance, ::cvmwflann::Index<Distance> >(index0, index, data, fin, dist);
}
...
bool Index::load(InputArray _data, const std::string& filename)
{
    ...
    ::cvmwflann::IndexHeader header = ::cvmwflann::load_header(fin);
    ...
    int idistType = 0;
    ::cvmwflann::load_value(fin, idistType);
    ...
    case FLANN_DIST_HAMMING:
        ...
        break;
    case FLANN_DIST_L2:
        loadIndex< ::cvmwflann::L2<float> >(this, index, data, fin);
        break;
    case FLANN_DIST_L1:
        loadIndex< ::cvmwflann::L1<float> >(this, index, data, fin);
        break;
#if MINIFLANN_SUPPORT_EXOTIC_DISTANCE_TYPES
    case FLANN_DIST_MAX:
        loadIndex< ::cvmwflann::MaxDistance<float> >(this, index, data, fin);
        break;
    case FLANN_DIST_HIST_INTERSECT:
        loadIndex< ::cvmwflann::HistIntersectionDistance<float> >(index, data, fin);
        break;
    case FLANN_DIST_HELLINGER:
        loadIndex< ::cvmwflann::HellingerDistance<float> >(this, index, data, fin);
        break;
    case FLANN_DIST_CHI_SQUARE:
        loadIndex< ::cvmwflann::ChiSquareDistance<float> >(this, index, data, fin);
        break;
    case FLANN_DIST_KL:
        loadIndex< ::cvmwflann::KL_Divergence<float> >(this, index, data, fin);
        break;
#endif
    default:
